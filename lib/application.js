...
module.exports = class Application extends Emitter { // koa继承来自于 Emitter
  ...
  callback() {
    ...
    const handleRequest = (req, res) => {
      const ctx = this.createContext(req, res); // 将获取到的req 和res 绑定到 app上面
  ...
  handleRequest(ctx, fnMiddleware) {
    ...
    onFinished(res, onerror); // HTTP请求关闭，完成或错误时执行回调
  ...
  createContext(req, res) { // 每个请求创建新的上下文对象
  ...
  onerror(err) { // 默认的错误处理程序  当用户不设置 app.on('error',fn)
